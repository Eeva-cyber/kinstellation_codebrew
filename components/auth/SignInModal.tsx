...
export function SignInModal({ defaultView = 'signin', onClose, onSuccess }: SignInModalProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (view === 'signin') handleSignIn(); else handleSignUp();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {view === 'signup' && (
            <div className="flex flex-col gap-1.5">
              <label className="text-white/50 text-xs tracking-widest uppercase">Confirm password</label>
              <input
                type="password"
        ...
        <p className="text-center text-xs mt-4 text-white/30">
          {view === 'signin' ? (
            <>
              ...
              {/* "Create one" navigates to full onboarding — not inline signup */}
              <button
                onClick={() => { onClose(); router.push('/onboarding'); }}
                className="text-amber-400/60 hover:text-amber-400/80 transition-colors"
              >
                Create one
              </button>
            </>
        ...
        {!showEmail ? (
          ...
          <div className="flex gap-2 mt-3 animate-fade-in">
            ...
            />
            <button type="button" onClick={handleEmailLink} disabled={loading}
